...
"""
<summary>
Static checks for the Jellyfin Dual Play Kodi add-on
...
def fail(message):
    """
    <summary>
    Record one failure for the final report.
    </summary>
...
def check(condition, message):
    ...
    <param name="message">What is wrong when it does not.</param>
    """
    if not condition:
        fail(message)
...
def check_addon_xml(root, addon):
    """
    <summary>
    Check addon.xml: the id, the version against VERSION and its shape, the provider, the xbmc.python import, every extension library on disk, the icon and the licence element.
    </summary>
    <param name="root">The project or stage root, where VERSION lives.</param>
...
def walk_text_files(root):
    """
    <summary>
    Every file under root, skipping the folders in SKIP_DIRS.
    </summary>
...
def check_python(root):
    """
    <summary>
    Compile every .py file, recording a failure for each syntax error.
    </summary>
    <param name="root">Where to start.</param>
    """
...
def check_house_rules(root):
    """
    <summary>
    Scan every text file for dashes, attribution trailers, credential shapes and private addresses, and every file name for a secret looking name.
    </summary>
    <param name="root">Where to start.</param>
    <remarks>
    This file exempts itself from the trailer, token and address checks because it has to spell them out.
    </remarks>
    """
...
def main(argv):
    """
    <summary>
    Run every check on the given root, or on this project, and print the outcome.
    </summary>
    <param name="argv">The command line; argv[1] is an optional root.</param>
    <returns>0 when clean, 1 when anything failed.</returns>
    """
